...
int findCorrectPassword(const char* arr_pwds[], int num_pwds) {
    int l[10] = { };//tạo mảng lưu số lần giống nhau giữa chuỗi đầu 2,3,4,..,chuỗi 2 với 3,4,5..,chuỗi 3 với 4,5,6
    ...
    char* b = new char[n];// tạo mảng lưu các giá trị có số lần xuất hiện nhiều nhất
    for (int i = 0; i < n + 1; i++) {
        for (int j = 0; j < num_pwds; j++)
        {
            if (l[j] >= max)// so sánh 2 chuỗi
            {
                b[i] = j;
            }
        }
